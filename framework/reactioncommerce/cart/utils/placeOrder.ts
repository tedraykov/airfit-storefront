...
export const placeOrder =
  (fetcher: Fetcher) =>
    async (placeOrderInput: PlaceOrderInput): Promise<Order> => {
      ...
      const reactionFulfillmentGroups = fulfillmentGroups.map(fg => (<ReactionOrderFulfillmentGroupInput>{
        data: {
        ...
        items: fg.items.map(item => (<OrderFulfillmentGroupItemInput>{
      ...
      const {
        ...
        variables: {
          input: <ReactionPlaceOrderInput>{
            ...
            payments: placeOrderInput.payments.map(payment => (<ReactionPaymentInput>{
